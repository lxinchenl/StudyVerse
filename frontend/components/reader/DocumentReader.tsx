...
import { ReaderChatPanel } from "@/components/reader/ReaderChatPanel";
...
import { initializeBackgroundChat, startBackgroundChat } from "@/lib/background-chat";
...
export function DocumentReader({
  ...
  async function handleAskAgent() {
    ...
    setChatOpen(true);
    await initializeBackgroundChat(userId);
    await startBackgroundChat(userId, prompt, {
      courseId,
      ...
      <div className="reader-layout">
        <aside className="reader-sidebar">
          ...
          <div className="panel-card highlight-card">
            ...
            <textarea
              className="reader-selection-input"
              value={selectedText}
              onChange={(e) => setSelectedText(e.target.value)}
              placeholder="划选或输入要问 Agent 的内容，点击按钮将原样发送…"
              rows={6}
            />
            <button
